...
def plot_function_values(values_dict, title):
    fig, ax = plt.subplots()
    # Define different markers and line styles to distinguish the two plots
    markers = ['o', 's']
    line_styles = ['-', '--']
    colors = ['b', 'r']
    ...
    for i, (label, values) in enumerate(values_dict.items()):
        ...
        ax.plot(iterations, values, color=colors[i], marker=markers[i], linestyle=line_styles[i], label=label)
