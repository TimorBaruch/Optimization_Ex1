...
class TestUnconstrainedMinimization(unittest.TestCase):
    def run_all_tests(self, name, f, x0=[1.0, 1.0], obj_tol=10**-12, param_tol=10**-8, max_iter=100,
        ...
        grad_location = f'({grad_x[0]:.4f}, {grad_x[1]:.4f})'
        newton_location = f'({newton_x[0]:.4f}, {newton_x[1]:.4f})'

        print(
            f'{"Gradient Descent":<20} {len(grad_history["path"]) - 1:<15} {grad_location:<26} {grad_f:<15.6f} {str(grad_success):<15}')
        print(
            f'{"Newton":<20} {len(newton_history["path"]) - 1:<15} {newton_location:<26} {newton_f:<15.6f} {str(newton_success):<15}')
        print()
