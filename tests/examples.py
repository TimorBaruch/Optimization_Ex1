...
def quadratic_example_1(x, is_hessian):
    # Q = np.array([[1, 0], [0, 1]])
    # f = x.T.dot(Q).dot(x)
    # g = 2 * Q.dot(x)
...
def quadratic_example_2(x, is_hessian):
...
def quadratic_example_3(x, is_hessian):
    ...
    h = 2 * Q if is_hessian else None
    return f, g, h
    # Q = np.array([[np.sqrt(3), 2], [-0.5, 0.5]]) @ np.array([[100, 0], [0, 1]]) @ np.array([[np.sqrt(3), 2], [-0.5, 0.5]])
    # f = np.dot(x, np.dot(Q, x))
    # g = 2 * np.dot(Q, x)
    # h = 2 * Q if is_hessian else None
    # return f, g, h

def rosenbrock_function(x, is_hessian):
...
def linear_function(x, is_hessian, a=[5, 5]):
    a = np.array(a)
    f = a.T.dot(x)
    g = a
...
def smoothed_triangle_function(x, is_hessian):
